...
ExclusiveGateway.prototype.run = function() {
  ...
  try {
    ...
    this._debug(`<${this.id}>`, `getInput error: ${error.message}`);
    return this.emit('error', error, this);
  }
...
function takeAll(outbound, input) {
  ...
  for (let i = 0; i < conditionalFlows.length; i++) {
    ...
    let conditionResult;
    try {
      conditionResult = sequenceFlow.evaluateCondition(input);
    } catch (error) {
      this._debug(`<${this.id}>`, `evaluateCondition error on flow <${sequenceFlow.id}>: ${error.message}`);
      return this.emit('error', error, this);
    }

    if (!takenFlow && conditionResult) {
